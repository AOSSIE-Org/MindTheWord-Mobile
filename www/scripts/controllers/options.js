...
import * as Hammer from '../../assets/js/hammer.min.js';
...
export class OptionCtrl {
  ...
  getYandexLimit(period){
    if(JSON.parse(this.stats['translatorWiseWordCount'])[0][this.currentMonth]){
      var limit;
      switch (period) {
        case 'monthly':
          limit = (JSON.parse(this.stats['translatorWiseWordCount'])[0][this.currentMonth]['Yandex'][2]/10000000)*100;
          break;
        case 'daily':
          limit = (JSON.parse(this.stats['translatorWiseWordCount'])[1][this.currentDay]['Yandex'][2]/1000000)*100;
          break;
      }
      if (limit > 100){
        return 100.00;
  ...
  save(data, successMessage) {
    FirebaseApp.ref('devices/' + localStorage.deviceID+'/options/').update(data).then(function(){
      $('.modal').modal('close');
      Materialize.toast(successMessage,1000);
    });
  ...
  deleteActivationData() {
    ...
    FirebaseApp.ref('devices/' + localStorage.deviceID+'/options/').update({
      translatorService: '',
   ...
   firebaseConfig(){
            ...
            if(deviceID == null){ // for browsers
                if(localStorage.deviceID == null){
                    deviceID = FirebaseApp.ref('devices/').push().key;
                    FirebaseApp.ref('devices/' + deviceID).set(default_device_info);
            ...
            var deviceRef = FirebaseApp.ref('devices/' + deviceID);
