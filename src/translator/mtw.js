...
import { getCurrentMonth,getCurrentDay } from './dateAndTimeHelpers';
...
export class ContentScript {
    ...
    translate() {
        var $this = this;
        FirebaseApp.ref(this.getUrl()).once('value').then(function(snapshot) {
            var device_info = snapshot.val();
            // 
            var res = device_info.options;
            console.log(res.sourceLanguage, res.targetLanguage);
            // alert(JSON.stringify(res.savedPatterns));     
            // window.localStorage.setItem('extensionID',res.extensionID);
            if (res.activation === true) {
                $this.ngramMin = Number.parseInt(res.ngramMin);
                $this.ngramMax = Number.parseInt(res.ngramMax);
                ...
                // console.log($this.stats['translatorWiseWordCount'],JSON.parse($this.stats['translatorWiseWordCount']));
                $this.wordToggles = Number.parseInt(res.wordToggles);
    ...
    injectCSS(cssStyle) {
        try {
            ...
            // style.type  = 'text/css';
            // style.href  = chrome.extension.this.getURL('/assets/css/MTWStyles.css');
    ...
    processTranslations(translationMap, userDefinedTMap) {
        ...
        //for quiz feature
        FirebaseApp.ref(this.getUrl() + '/options/').update({
        ...
        FirebaseApp.ref(this.getUrl() + '/options/savedPatterns/').once('value').then(function(snapshot) {
            var savedPatterns = JSON.parse(snapshot.val());
        ...
        for (let i = 0; i < translatedWords.length; i++) {
            translatedWords[i].addEventListener('click', (event) => {
    ...
    toggleElement(event, wordToggles, autoBlacklist) {
        ...
        if (autoBlacklist) {
            ...
            FirebaseApp.ref(this.getUrl() + '/options/toggleFrequency/').once('value').then(function(snapshot) {
                let data = snapshot.val();
                if (original in data) {
                    if (data[original] > wordToggles) {
                        // add word to blacklist
                        // chrome.storage.local.get('userBlacklistedWords', (result) => {
                        FirebaseApp.ref(this.getUrl() + '/options/userBlacklistedWords/').once('value').then(function(result) {
                            let blacklistedWords = JSON.parse(result.val()).slice(1, -1).split('|');
